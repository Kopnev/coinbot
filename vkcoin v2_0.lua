...
function imgui.OnDrawFrame()
	...
	--[[if price_windows_state.v then
		...
		imgui.Begin(u8'Настройка цен', price_windows_state)
		imgui.Text(u8'Введите сколько переферии у вас куплено или текущую цену за перефирию:')
		imgui.NewLine()

		imgui.BeginChild('acc', imgui.ImVec2(450, 210), true)
		imgui.Columns(3, 'mycolumns')
		imgui.Separator()
		imgui.Text(u8'Переферия') imgui.NextColumn()
		imgui.Text(u8'Кол-во') imgui.NextColumn()
		imgui.Text(u8'Цена') imgui.NextColumn()
		imgui.Separator()
		imgui.Text(u8'Клик мыши') imgui.NextColumn() imgui.InputFloat('##1', clicks,1) imgui.NextColumn() imgui.Text(''..clicks.v*0.0002 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Видеокарта') imgui.NextColumn() imgui.InputFloat('##2', video,1) imgui.NextColumn() imgui.Text(''..video.v*0.0008 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Стойка видеокарт') imgui.NextColumn() imgui.InputFloat('##3', stoika,1) imgui.NextColumn() imgui.Text(''..stoika.v*0.003 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Суперкомпьютер') imgui.NextColumn() imgui.InputFloat('##4', super,1) imgui.NextColumn() imgui.Text(''..super.v*0.01 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Сервер Arizona Games') imgui.NextColumn() imgui.InputFloat('##5', server,1) imgui.NextColumn() imgui.Text(''..server.v*0.04 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Квантовый компьютер') imgui.NextColumn() imgui.InputFloat('##6', kvant,1) imgui.NextColumn() imgui.Text(''..kvant.v*0.16 .. ' VKoin') imgui.NextColumn()
		imgui.Text(u8'Датацентр') imgui.NextColumn() imgui.InputFloat('##7', data,1) imgui.NextColumn() imgui.Text(''..data.v*1 .. ' VKoin')
		imgui.EndChild()
		imgui.NewLine()
		imgui.SameLine(360)
		if imgui.Button(u8'Сохранить', ImVec2(100, 30)) then
			...
			if inicfg.save(ini, directIni) then
				sampAddChatMessage('[BotVkCoin] {FFFFFF}Успешно сохранено.', 0xF1CB09)
			else sampAddChatMessage('[BotVkCoin] {f10000}Ошибка.', 0xF1CB09) end
	...
	if imgui.TreeNode(u8'Покупка улчшений') then
		...
		if imgui.TreeNode(u8'Выберите тип закупки') then
			imgui.RadioButton(u8'На что хватает', tip, 0)
			imgui.RadioButton(u8'Выборочно', tip, 1)
			imgui.TreePop()
		end

		if tip.v == 0 then
			if imgui.TreeNode(u8'Настройка закупки') then
			  imgui.Text(u8'Что покупать?')
				imgui.Text(u8'')
		   	imgui.Text(u8'Клик мыши') imgui.SameLine(200) imgui.Checkbox('##1', buy[1])
				imgui.Text(u8 "Видеокарта") imgui.SameLine(200) imgui.Checkbox('##2', buy[2])
				imgui.Text(u8 "Стойка видеокарт") imgui.SameLine(200) imgui.Checkbox('##3', buy[3])
				imgui.Text(u8 "Суперкомпьютер") imgui.SameLine(200) imgui.Checkbox('##4', buy[4])
				imgui.Text(u8 "Сервер Arizona Games") imgui.SameLine(200) imgui.Checkbox('##5', buy[5])
				imgui.Text(u8 "Квантовый компьютер") imgui.SameLine(200) imgui.Checkbox('##6', buy[6])
		...
		if imgui.TreeNode(u8'Установите задержку') then
